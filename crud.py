...
def insert_pc(ip, patrimonio, hostname, modelo, processador, memoria, armazenamento, q_monitores, observacoes):
    conn = connect()
    conn.execute("INSERT INTO computadores(ip, patrimonio, hostname, modelo, processador, memoria, armazenamento, q_monitores, observacoes)\
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", (ip, patrimonio, hostname, modelo, processador, memoria, armazenamento, q_monitores, observacoes))
    conn.commit()
    conn.close()
...
def insert_assignment(id_usuarios, id_computadores, data_atribuicao, data_troca):
    conn = connect()
    conn.execute("INSERT INTO atribuicoes(id_usuarios, id_computadores, data_atribuicao, data_troca)\
                VALUES (?, ?, ?, ?)", (id_usuarios, id_computadores, data_atribuicao, data_troca))
    conn.commit()
    conn.close()
...
def get_pcs_on_assignment():
    conn = connect()
    result = conn.execute("SELECT atribuicoes.id, usuarios.nome, computadores.modelo, computadores.patrimonio, atribuicoes.data_atribuicao, atribuicoes.data_troca\
                         FROM computadores\
...
#insert_assignment("1","1","25/08/2025", "none")
